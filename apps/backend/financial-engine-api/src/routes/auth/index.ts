...
import { StripeClient } from "@/providers/stripe/stripe-api";
import type { LinkTokenCreateAndExchangeRequest } from "@/providers/types";
...
import {
  ...
  LinkTokenCreateAndExchangeBodySchema,
  LinkTokenCreateAndSchema,
...
const createAndExchangeStripeTokenRoute = createRoute({
  method: "post",
  path: "/stripe/create-and-exchange",
  summary: "Create And Exchange Token (Stripe)",
  request: {
    body: {
      content: {
        "application/json": {
          schema: LinkTokenCreateAndExchangeBodySchema,
        },
      },
    },
  },
  responses: {
    200: {
      content: {
        "application/json": {
          schema: LinkTokenCreateAndSchema,
        },
      },
      description: "Retrieve Exchange",
    },
...
app.openapi(createAndExchangeStripeTokenRoute, async (c) => {
  const envs = env(c);

  const { accountId, refreshUrl, returnUrl, type } = await c.req.json();

  const api = new StripeClient({
    kv: c.env.KV,
    envs,
  });

  const req: LinkTokenCreateAndExchangeRequest = {
    accountId,
    refreshUrl,
    returnUrl,
    type,
  ...
  const data = await api.itemTokenCreateAndExchange(req);
